...
class Murid extends CI_Controller{
      ...
      $file = $upload_data['file_path'].$upload_data['file_name'];
	    ...
	    $highestColumnIndex = PHPExcel_Cell::columnIndexFromString($highestColumn);
          ...
          for ($i=0; $i < sizeof($program) ; $i++) {
            if (strcasecmp($dataArr[$row][3],$program[$i]['nama']) == 0 ) {
              $dataArr[$row][3] = $program[$i]['id'];
            }
          }
          $dataArr[$row][4] = $kelas;
          $dataArr[$row][5] = $tahun_ajaran;
          $dataArr[$row][6] = gmdate("Y-m-d H:i:s", time()+60*60*7);
		    }
			}
  ...
  private function renderMurid($data){
    foreach ($data as $datamurid => $dat) {
      $rebuild_data = array('no_induk'        => $dat[0],
														'nama'            => $dat[1],
		 												'no_telp'         => $dat[2],
                            'program'         => $dat[3],
                            'kelas'           => $dat[4],
